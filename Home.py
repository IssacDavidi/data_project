...
if st.session_state.show_data_table:
    st.dataframe(df)
    with st.expander(st.image('https://github.com/IssacDavidi/data_project/blob/main/photos/cat2.jpg?raw=true')):
...
if st.session_state.show_insights:
    ...
    with col1:
        ...
        st.plotly_chart(fig3) # Top authors
        st.plotly_chart(fig4) #Sub Category Count
        st.subheader('Bar Charts')
        st.plotly_chart(fig1) #Physical VS Digital
        st.plotly_chart(fig2) # Count Categories
